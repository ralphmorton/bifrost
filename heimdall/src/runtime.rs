...
pub fn exec(registry: &Registry, module_id: &str, label: &str, json: &serde_json::Value) -> ExecutionResult {
...
#[inline]
fn or_error<T, E>(res: Result<T, E>, prefix: &'static str) -> Option<T> where E : ToString {
    match res {
        Ok(v) => Some(v),
        Err(e) => {
